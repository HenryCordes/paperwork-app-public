...
import { useScan, ScanResult, buildReceiptFileName } from "../../../hooks/useScan";
...
const ExpenseEditPage: React.FC = () => {
  ...
  const [scannedFileLocation, setScannedFileLocation] = useState<
    string | undefined
  >(undefined);
  ...
  const handleRemoveDocument = () => {
    ...
    setScannedFileLocation(undefined);
  ...
  const handleScan = async () => {
    try {
      ...
      if (result) {
        ...
        setScannedFileLocation(result.fileLocation);
        ...
        // Convert the scanned image to a File object for upload - only
        // needed when the scan itself didn't already upload the image
        // (the LLM path uploads as a side effect of extraction; reuse that
        // location on save instead of uploading the same image twice).
        if (!result.fileLocation) {
          try {
            ...
            const fileName = buildReceiptFileName(
              result.rawText[0] || null,
              result.receiptInfo?.date ?? null,
            );
  ...
  const applyScannedValues = () => {
    if (scanResult && scanResult.receiptInfo) {
      ...
      if (selectedValues.date) {
        updates.expenseDate = formatISO(
          scanResult.receiptInfo.date instanceof Date &&
            !Number.isNaN(scanResult.receiptInfo.date.getTime())
  ...
  const handleSubmit = async () => {
    try {
      // Reuse the LLM scan's upload if we have one; otherwise upload the
      // selected file as before.
      let fileLocation = scannedFileLocation ?? formData.expenseFile;
      if (!scannedFileLocation && selectedFile) {
                  ...
                  >
                    ...
                    {isScanning ? (
                      <>
                        ...
                        {useLlmExtraction ? " AI leest bon" : " Scannen..."}
                      </>
                    ) : (
                      "Bon Scannen"
                    )}
