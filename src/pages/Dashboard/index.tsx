...
const Dashboard: React.FC = () => {
  ...
  const calculateSummary = () => {
    // Prefer server-computed summaryMetrics (real-time path) when available.
    // The hybrid-calculation response includes these and they match the export.
    if (dashboardData?.data?.summaryMetrics) {
      return {
        totalRevenue: dashboardData.data.summaryMetrics.totalRevenue,
        totalExpenses: dashboardData.data.summaryMetrics.totalExpenses,
        netProfit: dashboardData.data.summaryMetrics.netProfit,
      };
    }

    // Fall back to summing rawData for pre-calculated / dynamic responses.
    if (
    ...
    return dashboardData.data.rawData.reduce(
      (acc, dataPoint) => ({
                      ...
                      <DashboardChartContainer>
                        {dashboardData?.data && (
                          <FinancialChart
                            ...
                            rawData={dashboardData.data.rawData}
                          />
                        )}
                      </DashboardChartContainer>
                    </IonCardContent>
