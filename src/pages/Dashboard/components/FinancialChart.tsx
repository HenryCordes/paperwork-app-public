...
import { RawDataPoint } from "../../../api/types/dashboard";
...
const FinancialChart: React.FC<FinancialChartProps> = ({
  ...
  useEffect(() => {
    ...
    let turnoverData = chartData.turnover;
    let expensesData = chartData.expenses;

    if (rawData && Array.isArray(rawData)) {
      turnoverData = rawData.map((point) => point.totalRevenue || 0);
      expensesData = rawData.map((point) => point.totalExpenses || 0);
    }

    const totalRevenue = summaryTotals
      ? summaryTotals.totalRevenue
      : turnoverData.reduce((accumulator, currentValue) => {
          return accumulator + currentValue;
        }, 0);

    const totalExpenses = summaryTotals
      ? summaryTotals.totalExpenses
      : expensesData.reduce((accumulator, currentValue) => {
          return accumulator + currentValue;
        }, 0);
    ...
    setChartDataState({
      ...
      datasets: [
        {
          label: `Omzet: ${formatCurrency(totalRevenue)}`,
          data: turnoverData,
        ...
        {
          label: `Uitgaven: ${formatCurrency(totalExpenses)}`,
          data: expensesData,
  ...
  }, [labels, chartData, isDarkMode, summaryTotals, rawData]);
